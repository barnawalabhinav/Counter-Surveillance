...
public:
    ...
    inline void createScatterPlot(const std::vector<T2> &y, const char *point_type = "O", const double point_size = 1.0, const char *title = "", const char *point_color = "auto", const bool set_range = false)
    {
        ...
        if (point_color == "auto")
            ...
            fprintf(gnuplotPipe, "\"%s\" using 1:2 with points pointtype '%s' pointsize %f linecolor rgb '%s' title '%s'", filename.c_str(), point_type, point_size, point_color, title);
    ...
    inline void createPlot(const std::vector<T2> &y, const char *line_title = "", const char *line_color = "auto", const MarkerStyle marker = None, const double point_size = 1.0, const double line_width = 1.0, const LineStyle line_style = SOLID, const bool set_range = false)
    {
        ...
        fprintf(gnuplotPipe, "plot ");
        if (line_color == "auto")
            fprintf(gnuplotPipe, "\"%s\" using 1:2 smooth unique with linespoints pointtype %d pointsize %f linetype %d linewidth %f title '%s'", filename.c_str(), marker, point_size, line_style, line_width, line_title);
        else
            fprintf(gnuplotPipe, "\"%s\" using 1:2 smooth unique with linespoints pointtype %d pointsize %f linetype %d linewidth %f linecolor rgb '%s' title '%s'", filename.c_str(), marker, point_size, line_style, line_width, line_color, line_title);
    ...
    inline void fillBetween(const std::vector<T2> &ub, const std::vector<T2> &lb, const char *color = "auto", const double alpha = 0.2)
    {
        ...
        if (color == "auto")
            ...
            fprintf(gnuplotPipe, ", \"%s\" using 1:2:3 with filledcurves linecolor rgb '%s' fill transparent solid %f title ''", filename.c_str(), color, alpha);
    ...
    inline void createLinePlot3D(const std::vector<T1> &x, const std::vector<T2> &y, const std::vector<T3> &z, const char *line_title = "", const char *line_color = "auto", const MarkerStyle marker = None, const double point_size = 1.0, const double line_width = 1.0, const LineStyle line_style = SOLID, const bool set_hidden3D = true)
    {
        ...
        if (line_color == "auto")
            fprintf(gnuplotPipe, "\"%s\" using 1:2:3 with linespoints pointtype %d pointsize %f linetype %d linewidth %f title '%s'", filename.c_str(), marker, point_size, line_style, line_width, line_title);
        else
            fprintf(gnuplotPipe, "\"%s\" using 1:2:3 with linespoints pointtype %d pointsize %f linetype %d linewidth %f linecolor rgb '%s' title '%s'", filename.c_str(), marker, point_size, line_style, line_width, line_color, line_title);
    ...
    //     else
    //         fprintf(gnuplotPipe, "\"%s\" using 1:2:3 with points pointtype '%s' pointsize %f linecolor rgb '%s' title '%s'", filename.c_str(), point_type, point_size, point_color, title);
